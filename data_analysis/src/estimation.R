# version: 7/08/2023
# Splines are recentered, and use of a penalization.
# Packages required: Matrix, irlba, Rfast (if the fastProd option is used)
...
AFM <- function( x, q = 1, K = 4, Niter = 10, lambda = 0.1, method = "Hungarian", fastProd = FALSE, random_p = NULL, Kvec = NULL ){
  ...
  for( it in 1:Niter ){
    ...
    # reorder factors according to mse:
    # note that this is slightly different from ordering with diff of cumulative Explained variance
    # because there is some correlation between factors
